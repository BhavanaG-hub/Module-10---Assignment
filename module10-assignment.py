...
df_no_missing['last_name'].fillna('Unknown', inplace=True)
df_no_missing['phone'].fillna('0000000000', inplace=True)
df_no_missing['loyalty_status'].fillna('Bronze', inplace=True)
df_no_missing['age'].fillna(df_no_missing['age'].median(), inplace=True)
...
df_text_cleaned['phone'] = df_text_cleaned['phone'].str.replace(
...
df_no_duplicates['purchase_frequency_category'] = df_no_duplicates['total_purchases'].apply(categorize)
...
print("- Data Type Issues: Dates, currency formatting, numeric fields")
